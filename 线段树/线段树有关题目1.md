...
[C-牛牛的Link Power II_牛客2025秋季算法编程训练联赛3-提升组](https://ac.nowcoder.com/acm/contest/120456/C)
...
public class Main {
    ...
    public static void solve() throws IOException {
        ...
        Long sum = 0L;ST seg = new ST(n);

        for(int i = 0;i < n;i++){
            if(s.charAt(i) == '1'){//i + 1
                seg.modify(1,1,n,i + 1,1);
                long x = seg.get(i + 1,n);
                sum = (sum + x) % MOD;
            }
        }
        pw.println(sum);
        for(int i = 0;i < m;i++){
            StringTokenizer st = new StringTokenizer(br.readLine());
            int a = Integer.parseInt(st.nextToken()),pos = Integer.parseInt(st.nextToken());
            long x = seg.get(pos,n);
            if(a == 1){
                seg.modify(1,1,n,pos,1);
                sum = (sum + x) % MOD;
            }else{
                seg.modify(1,1,n,pos,0);
    ...
    public static class ST {
        ...
        public void maintain(int id) {
            arr[id].cnt = arr[id << 1].cnt + arr[(id << 1) | 1].cnt;
            arr[id].sum = arr[id << 1].sum + arr[(id << 1) | 1].sum;
        }

        public void modify(int id,int l,int r,int a,int b){
            if(l == r) {
                if(b == 1) {
                    arr[id].sum = l;
                }else{
            ...
            if(a <= mid)modify(id << 1, l, mid, a,b);
            else modify((id << 1) | 1, mid + 1, r,a,b);
            maintain(id);
            return;
        }
        public long get(int pos,int n){
            long l = queryCnt(1,1,n,1,pos - 1),r = queryCnt(1,1,n,pos + 1,n);
            long sumL = querySum(1,1,n,1,pos - 1),sumR = querySum(1,1,n,pos + 1,n);
            //pos * l - sumL + (sumR - r * pos)
            return (((pos * l % MOD - sumL) % MOD   + (sumR - r * pos % MOD) % MOD) % MOD + MOD) % MOD;
        }
        ...
        public int queryCnt(int id,int l,int r,int x,int y){
            if(x > y)return 0;
            if (x > r || y < l) return 0;
            if(x <= l && r <= y)return arr[id].cnt;
            int mid = (l + r) >> 1;int res = 0;
            if(x <= mid)res += queryCnt(id << 1, l, mid, x, y);
            if(y > mid)res += queryCnt((id << 1) | 1, mid + 1, r, x, y);
            return res;
        }
        public long querySum(int id,int l,int r,int x,int y){
            if(x > y)return 0;
            if (x > r || y < l) return 0;
            if(x <= l && r <= y)return arr[id].sum;
            int mid = (l + r) >> 1;long res = 0;
            if(x <= mid)res += querySum(id << 1, l, mid, x, y);
            if(y > mid)res += querySum((id << 1) | 1, mid + 1, r, x, y);
            return res;
        }
    }
